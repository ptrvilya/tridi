...
def preprocess(cfg):
    ...
    orig_objects_path = Path(cfg.behave.orig_objects_path)
    ...
    for sequence in tqdm.tqdm(sequences, total=len(sequences), ncols=80):
        ...
        for i in range(T):
            ...
            if cfg.align_with_joints:
                ...
                shoulderblades = sbj_joints[i][2] - sbj_joints[i][1]  # pelvis
                z = np.array([0, 0, 1])
        ...
        if seq_object in ["chairblack", "chairwood"]:
            ...
            suffix = "_f1000"
        obj_dec_mesh = orig_objects_path / seq_object / f"{seq_object}{suffix}.ply"
        ...
        for index, t_stamp in enumerate(t_stamps):
            ...
            obj_mesh_v = R.apply(obj_mesh_v - rot_center) + rot_center + t
        ...
        # ============ 6 calculate rotation for object ============
        # load canonicalized object mesh
        obj_mesh = trimesh_load(can_objects_path / f"{seq_object}.ply")
